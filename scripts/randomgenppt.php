...
if(isset($_REQUEST['type']))
{
	...
	if($_REQUEST['type']=='r')
	{
        $message = "Random Generate PPT By ".$_SESSION['name'];
        
        UserClickLog($conn,$_SESSION['cmpid'],'0',$_SESSION['name'],$useragent,$ipaddress,$agent_detail,$message,$ISP);

		$appnd = " ORDER BY RAND() limit 60";
	}
	if($_REQUEST['type']=='rp')
	{
		$message = "Random Powerteam-Generate PPT By ".$_SESSION['name'];
        
        UserClickLog($conn,$_SESSION['cmpid'],'0',$_SESSION['name'],$useragent,$ipaddress,$agent_detail,$message,$ISP);
        
		$Query_pt="SELECT GROUP_CONCAT(id order by rand()) ids FROM ri_powerteam where STATUS=1";
		$resultPT= mysqli_query($conn,$Query_pt);
		$rowsPT = mysqli_num_rows($resultPT);
		if($rowsPT > 0)
		{
			$fetchPT=mysqli_fetch_array($resultPT);
			$ptid = $fetchPT['ids'];
		}

			$appnd = " ORDER BY FIELD(rc.ptid,".$ptid."), rand() ";
	}
	if($_REQUEST['type']=='pre')
	{
        $message = "user-preview-Generate PPT By ".$_SESSION['name'];
        
        UserClickLog($conn,$_SESSION['cmpid'],'0',$_SESSION['name'],$useragent,$ipaddress,$agent_detail,$message,$ISP);
        
		$appnd = " AND rc.cmpid=".$_SESSION['cmpid'];
	}
	...
	WHERE rc.STATUS=1 and rc.cname<>'Admin' AND rci.`status`=1 ".$appnd." ";	
...
echo " Something Wnt Wrong! Please Try again\n";
...
foreach($arrData as $value )
{
	...
	$uspLimit = $value['usp']; //limitText($value['usp'],50);
...
// Create a shape (text) for USP
	...
	$shape->setHeight(250);
	$shape->setWidth(490);
	$shape->setOffsetX(40);	// for content
